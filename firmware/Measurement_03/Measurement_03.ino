...
byte mode = 0;                    // state machine: 0=menu, 1=detect, 2=burst
volatile unsigned int count = 0;  // error counter
unsigned int prev_count = 100;    // prev error counter -> detect changes -> update screen
unsigned long lastUI = 5000;      // timestamp of last display refresh
unsigned long duration = 0;       // counter for latency measurement
int baseline = 0;                 // low level (no input signal) -> noise floor
unsigned long latency[10];        // last 10 latency measurements in microseconds
float avgLatency = 0;             // average of last 10 latency measurements
float offset = 340;               // latency compensation in us -> experimentally choosen using loop-back


void setup() {
  Serial.begin(115200);  // for debugging
  startup();             // get everything ready
}



void loop() {  // basic state machine with 3 states
